...
export default function App() {
  ...
  // Use localStorage to track shown notifications so they don't repeat on refresh
  const getShownNotifIds = () => {
    try {
      return new Set(JSON.parse(localStorage.getItem('shownNotifIds') || '[]'));
    } catch { return new Set(); }
  };
  const saveShownNotifId = (id) => {
    try {
    ...
    const loadUserData = async () => {
      try {
        ...
        if (cloudSettings) {
          ...
          // Sync progress: Firebase is the SINGLE SOURCE OF TRUTH
          if (cloudSettings.progress && Object.keys(cloudSettings.progress).length > 0) {
            ...
            // Firebase empty (after reset) - CLEAR local progress too for clean start
            setProgress({});
            const userProgressKey = `studyProgress_${session.licenseKey}`;
...
// AdminDashboard is now lazy loaded from ./components/AdminDashboard.jsx
